...
const PersonalDetails = (props: Props) => {
  ...
  const validate = () => {
    const { error } = personalDetailsSchema.validate(form, { abortEarly: false })
    ...
    <div className='w-full xs:p-4 md:p-8 bg-[#FEFEFE] text-black rounded-[15px] border border-[#E8E8E9] flex flex-col justify-center items-center xs:gap-4 md:gap-7'>
      ...
      <div className='grid xs:grid-cols-1 w-full md:grid-cols-2 xs:gap-3 md:gap-7 max-w-[1000px] px-2'>
        ...
        />
          ...
          onChange={(e: any) =>
            setProfileData({ ...profileData, armed_force_status_id: e.target.value })
          }
          options={ARMY_STATUS}
          titleStyle={{ fontSize: 14 }}
          style={{ fontSize: 16 }}
        ...
        />
          ...
          onChange={(e: any) =>
            setProfileData({ ...profileData, first_generation_id: e.target.value })
          }
